...
fixed4 _BackColor;
fixed4 _BackShadeColor;
sampler2D _BackMainTex; float4 _BackMainTex_ST;
sampler2D _BackShadeTexture;
half _BumpScale;
sampler2D _BumpMap;
...
inline float4 back_CalculateOutlineVertexClipPosition(appdata_full v)
{
    float outlineTex = tex2Dlod(_OutlineWidthTexture, float4(TRANSFORM_TEX(v.texcoord, _BackMainTex), 0, 0)).r;
...
float4 back_frag_forward(v2f i) : SV_TARGET
{
#ifdef MTOON_CLIP_IF_OUTLINE_IS_NONE
    #ifdef MTOON_OUTLINE_WIDTH_WORLD
    #elif MTOON_OUTLINE_WIDTH_SCREEN
    ...
    // uv
    float2 mainUv = TRANSFORM_TEX(i.uv0, _BackMainTex);
    ...
    // main tex
    half4 mainTex = tex2D(_BackMainTex, mainUv);
    
    // alpha
    half alpha = 1;
#ifdef _ALPHATEST_ON
    alpha = _BackColor.a * mainTex.a;
    alpha = (alpha - _Cutoff) / max(fwidth(alpha), EPS_COL) + 0.5; // Alpha to Coverage
    clip(alpha - _Cutoff);
    alpha = 1.0; // Discarded, otherwise it should be assumed to have full opacity
#endif
    ...
    // Albedo color
    half4 shade = _BackShadeColor * tex2D(_BackShadeTexture, mainUv);
    half4 lit = _BackColor * mainTex;
    half3 col = lerp(shade.rgb, lit.rgb, lightIntensity);
